...
function init() {
  const setContentHeight = function () {
    if ($(".HIGHLIGHT").attr("id") === "projects") {
      $(".owl-carousel .containerContentBox").attr(
        "style",
        "height:fit-content"
      );
      ...
      var elements = document.querySelectorAll(
        ".owl-carousel .containerContentBox"
      );
      for (var index = 0; index < elements.length; index++)
        height =
          height < elements[index].offsetHeight
            ? elements[index].offsetHeight
            : height;
  ...
  $(".bottomNav").on("click", "div", function () {
    ...
    if (SelectedID === "projects") setTimeout(setContentHeight, 300);
  });
